...
class GradCam():
    ...
    def generate_adv_add_cam(self, input_image, target_class=None):
        # Full forward pass
        # conv_output is the output of convolutions at specified layer
        # model_output is the final output of the model (1, 1000)
        # input image [1, 3, 224, 224]
        ...
        for i in range(self.attack_iter):
            ...
            adv = F.relu(adv) - F.relu(attention_map.repeat(1, 3, 1, 1)) - (
                        F.relu(-adv) - F.relu(-attention_map.repeat(1, 3, 1, 1)))
